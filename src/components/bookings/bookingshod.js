...
const BookingsHod = () => {
  ...
  const getBookingData = async () => {
    try {
      const token=localStorage.getItem('jwttoken')
      const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/bookingHod`, {
       // withCredentials: true, // include credentials in the request
        headers: {
  ...
  const updateBooking = async (bookingId, isApproved) => {
    ...
    try {
      const token=localStorage.getItem('jwttoken')
      const response = await axios.put(`${process.env.REACT_APP_API_BASE_URL}/bookingsEdit/${bookingId}`, {
    ...
    <>
      ...
      <div className="mt-6 min-h-screen">
        <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-3xl xl:text-3xl text-center text-gray-800 font-black leading-7 ml-3 md:leading-10">
          Booking<span className="text-rose-700"> Requests</span>  </h1>
